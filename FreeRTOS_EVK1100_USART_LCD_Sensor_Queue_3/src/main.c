...
static xQueueHandle DIP204_Queue_Array[3];

void DIP204_Queue_Send (xQueueHandle Queue, struct DIP204_Message * Message)
{
	xQueueSendToBack (Queue, Message, 100);
}

void DIP204_Send_String (xQueueHandle Queue, int Row, int Col, char * Str)
{
	...
	while (*Str)
	{
		...
		DIP204_Queue_Send (Queue, &Message);
		Str = Str + 1;
...
static void DIP204_Initialize ()
{
	...
	DIP204_Queue_Array[0] = xQueueCreate (DIP204_Queue_Count, sizeof(struct DIP204_Message));
	DIP204_Queue_Array[1] = xQueueCreate (DIP204_Queue_Count, sizeof(struct DIP204_Message));
	DIP204_Queue_Array[2] = xQueueCreate (DIP204_Queue_Count, sizeof(struct DIP204_Message));
}
...
void Visual_Task_Function (void * Arguments)
{
	...
	while (1)
	{
		...
		vTaskDelay (1);
		if (xQueueReceive (DIP204_Queue_Array[0], Message, 100))
		{
			dip204_set_cursor_position (Message->Column, Message->Row);
			dip204_write_data (Message->Character);
		}
		if (xQueueReceive (DIP204_Queue_Array[1], Message, 100))
		{
			dip204_set_cursor_position (Message->Column, Message->Row);
			dip204_write_data (Message->Character);
		}
		if (xQueueReceive (DIP204_Queue_Array[2], Message, 100))
		{
			dip204_set_cursor_position (Message->Column, Message->Row);
			dip204_write_data (Message->Character);
		}
...
void Sensor_Task_Function (void * Parameters)
{
	...
	while (1)
	{
		...
		DIP204_Send_String (Config->Queue, Config->Row, Config->Column, Buffer);
	}
}
...
int main()
{
	...
	struct Sensor_Config Temperature_Config = {.Queue = DIP204_Queue_Array[0], .Channel = ADC_TEMPERATURE_CHANNEL, .Row = 1, .Column = 8};
	struct Sensor_Config Potentiometer_Config = {.Queue = DIP204_Queue_Array[1], .Channel = ADC_POTENTIOMETER_CHANNEL, .Row = 2, .Column = 8};
	struct Sensor_Config Light_Config = {.Queue = DIP204_Queue_Array[2], .Channel = ADC_LIGHT_CHANNEL, .Row = 3, .Column = 8};

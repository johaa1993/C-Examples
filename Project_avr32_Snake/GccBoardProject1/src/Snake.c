...
inline void Snake_Ring_Pointer_Forward (int * Index)
{
	(*Index)++;
	if ((*Index) == Snake_Body_Max_Count)
	{
		(*Index) = 0;
	}
}
...
struct Snake
{
	...
	struct Segment Body[Snake_Body_Max_Count];
};
...
{
	//No reason to excite the ring buffer if the snake is not moving.
	if (S->Direction != Snake_Direction_None)
	{
		int Head_Pointer_Old = S->Head_Pointer;
...
{
	return S->Body[S->Head_Pointer].X;
}

int Snake_Head_Y (struct Snake * S)
{
	return S->Body[S->Head_Pointer].Y;
}

int Snake_Tail_X (struct Snake * S)
{
	return S->Body[S->Tail_Pointer].X;
}

int Snake_Tail_Y (struct Snake * S)
{
	return S->Body[S->Tail_Pointer].Y;
}
...
int Snake_Collision_Wall (struct Snake * S, int Width, int Height)
{
	return
	(
		(Snake_Head_X (S) >= Width)  ||
		(Snake_Head_X (S) < 0)       ||
		(Snake_Head_Y (S) >= Height) ||
		(Snake_Head_Y (S) < 0)
	);
}
